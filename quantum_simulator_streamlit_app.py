...
# ── Right column ──────────────────────────────────────────────────────────────
with col_main:
    tab1,tab2,tab3,tab4=st.tabs(["Circuit Diagram","Probabilities","Entanglement","State Vector"])
    ...
    with tab4:
        st.markdown(
            ...
            f"margin-bottom:0.6rem'>// Full quantum state |ψ⟩ — amplitude, phase, and Bloch components "
            f"for all {2**sim_n} basis states</p>", unsafe_allow_html=True)

        # ── View selector ──────────────────────────────────────────────────
        sv_view = st.radio(
            "Representation",
            ["Amplitude & Phase", "Complex Plane", "Bloch Components"],
            horizontal=True, key="sv_view_radio",
            label_visibility="collapsed")

        if sv_view == "Amplitude & Phase":
            # Bar chart: amplitude magnitude + phase colour
            n_states = 2**sim_n
            amps     = np.abs(state_disp)
            phases   = np.angle(state_disp)          # −π … +π
            phase_norm = (phases + np.pi) / (2*np.pi) # 0…1 for colormap

            fig, (ax_amp, ax_ph) = plt.subplots(2, 1, figsize=(10, 5.5),
                                                  gridspec_kw={"height_ratios":[3,1.4]})
            fig.patch.set_facecolor(BG)

            # top: amplitude bars coloured by phase
            colours = plt.cm.hsv(phase_norm)
            colours[:,3] = 1.0 if is_running else 0.45
            ax_amp.set_facecolor(SIDEBAR)
            bars = ax_amp.bar(range(n_states), amps, color=colours, edgecolor=BORDER, linewidth=0.6)
            ax_amp.set_ylim(0, 1.08)
            ax_amp.set_ylabel("|amplitude|", color=DIM, fontsize=9, fontfamily="monospace")
            ax_amp.set_xticks(range(n_states))
            fs = max(6, 10-sim_n)
            ax_amp.set_xticklabels(basis_all, rotation=55, ha="right",
                                   fontfamily="monospace", fontsize=fs, color=WHITE)
            ax_amp.tick_params(colors=DIM, labelsize=8); ax_amp.spines[:].set_color(BORDER)
            ax_amp.set_title("// |amplitude| coloured by phase",
                             color=GREEN, fontsize=8, fontfamily="monospace", loc="left", pad=6)
            for i, (bar, a) in enumerate(zip(bars, amps)):
                if a > 0.02:
                    ax_amp.text(bar.get_x()+bar.get_width()/2, a+0.02,
                                f"{a:.3f}", ha="center", va="bottom",
                                color=WHITE if is_running else DIM,
                                fontsize=max(6,8-sim_n), fontfamily="monospace")

            # bottom: phase dots
            ax_ph.set_facecolor(BG)
            scat_alpha = 1.0 if is_running else 0.4
            sc = ax_ph.scatter(range(n_states), phases,
                               c=phases, cmap="hsv", vmin=-np.pi, vmax=np.pi,
                               s=40, zorder=3, alpha=scat_alpha, edgecolors=BORDER, linewidths=0.5)
            ax_ph.axhline(0, color=BORDER, lw=0.8, linestyle="--")
            ax_ph.set_xlim(-0.5, n_states-0.5)
            ...
            ax_ph.set_xticks([]); ax_ph.set_ylabel("phase φ", color=DIM, fontsize=8, fontfamily="monospace")
            ax_ph.tick_params(colors=DIM, labelsize=7); ax_ph.spines[:].set_color(BORDER)

            cb = plt.colorbar(sc, ax=ax_ph, orientation="vertical", fraction=0.015, pad=0.02)
        ...
        elif sv_view == "Complex Plane":
            # Scatter on the unit disk — one point per basis state
            n_states = 2**sim_n
            ...
            ax.plot(np.cos(theta_circ), np.sin(theta_circ), color=BORDER, lw=1.0, zorder=1)
            ax.axhline(0, color=BORDER, lw=0.7, linestyle="--", zorder=1)
            ax.axvline(0, color=BORDER, lw=0.7, linestyle="--", zorder=1)

            # Amplitude rings
            for r in [0.25, 0.5, 0.75]:
                ax.plot(r*np.cos(theta_circ), r*np.sin(theta_circ),
                        color=BORDER, lw=0.4, linestyle=":", zorder=1)

            alpha_pts = 1.0 if is_running else 0.45
            re_vals = state_disp.real; im_vals = state_disp.imag
            probs_norm = probs_d / (probs_d.max() + 1e-12)

            # Draw arrows from origin + scatter points
            for i in range(n_states):
                if probs_d[i] > 1e-6:
                    colour = plt.cm.hsv((np.angle(state_disp[i]) + np.pi) / (2*np.pi))
                    ax.annotate("", xy=(re_vals[i], im_vals[i]), xytext=(0,0),
                                arrowprops=dict(arrowstyle="-|>",
                                                color=(*colour[:3], alpha_pts*0.6),
                                                lw=1.2))

            sc = ax.scatter(re_vals, im_vals,
                            c=np.angle(state_disp), cmap="hsv", vmin=-np.pi, vmax=np.pi,
                            s=[max(20, p*400) for p in probs_d],
                            alpha=alpha_pts, edgecolors=BORDER, linewidths=0.5, zorder=4)

            # Label significant states
            for i in range(n_states):
                if probs_d[i] > 0.03:
                    ax.annotate(basis_all[i],
                                (re_vals[i], im_vals[i]),
                                textcoords="offset points", xytext=(6,4),
                                color=CYAN, fontsize=max(6, 9-sim_n),
                                fontfamily="monospace", alpha=alpha_pts)

            cb = plt.colorbar(sc, ax=ax, fraction=0.04, pad=0.04)
            cb.set_label("phase φ", color=DIM, fontsize=8, fontfamily="monospace")
            cb.set_ticks([-np.pi, -np.pi/2, 0, np.pi/2, np.pi])
            cb.set_ticklabels(["-π","-π/2","0","π/2","π"])
            cb.ax.tick_params(colors=DIM, labelsize=7)

            ax.set_xlim(-1.15, 1.15); ax.set_ylim(-1.15, 1.15)
            ax.set_xlabel("Re(α)", color=DIM, fontsize=9, fontfamily="monospace")
            ax.set_ylabel("Im(α)", color=DIM, fontsize=9, fontfamily="monospace")
            ax.set_title("// complex plane — unit disk",
                         color=GREEN, fontsize=8, fontfamily="monospace", loc="left", pad=6)
            ax.tick_params(colors=DIM, labelsize=8); ax.spines[:].set_color(BORDER)
            ax.set_aspect("equal")
            ...
            # Summary metrics below the plot
            entropy = -np.sum(probs_d * np.log2(probs_d + 1e-15))
            purity  = float(np.sum(probs_d**2))
            n_sig   = int(np.sum(probs_d > 0.01))
            m1, m2, m3 = st.columns(3)
            for col, label, val in zip([m1,m2,m3],
                ["Von Neumann entropy","State purity","Significant states"],
                [f"{entropy:.4f} bits", f"{purity:.4f}", f"{n_sig} / {2**sim_n}"]):
                col.markdown(
                    f"<div style='background:{PANEL};border:1px solid {BORDER};"
                    f"border-left:3px solid {CYAN};border-radius:4px;"
                    f"padding:0.5rem 0.8rem;font-family:JetBrains Mono,monospace'>"
                    f"<div style='color:{DIM};font-size:0.58rem;letter-spacing:1.5px;"
                    f"text-transform:uppercase'>{label}</div>"
                    f"<div style='color:{YELLOW};font-size:1rem;font-weight:700'>{val}</div>"
                    f"</div>", unsafe_allow_html=True)

        else:  # Bloch Components
            # For each qubit show ⟨X⟩, ⟨Y⟩, ⟨Z⟩ expectation values
            def bloch_vector(state, n, qubit):
                """Compute single-qubit Bloch vector by tracing out other qubits."""
                N = 2**n
                rho = np.zeros((2,2), dtype=complex)
                for idx in range(N):
                    bit = (idx >> (n-1-qubit)) & 1
                    for jdx in range(N):
                        bit2 = (jdx >> (n-1-qubit)) & 1
                        # Check all other bits match
                        mask = ~(1 << (n-1-qubit)) & (N-1)
                        if (idx & mask) == (jdx & mask):
                            rho[bit, bit2] += state[idx] * np.conj(state[jdx])
                sx = 2*rho[0,1].real
                sy = 2*rho[0,1].imag   # = -2*Im(ρ₀₁)... standard convention
                sz = (rho[0,0] - rho[1,1]).real
                return float(sx), float(-sy), float(sz)

            bloch_data = [bloch_vector(state_disp, sim_n, q) for q in range(sim_n)]
            xs = [b[0] for b in bloch_data]
            ys = [b[1] for b in bloch_data]
            zs = [b[2] for b in bloch_data]

            fig, axes = plt.subplots(1, 3, figsize=(10, 3.8))
            fig.patch.set_facecolor(BG)
            alpha_b = 1.0 if is_running else 0.4
            qubit_labels = [f"q{i}" for i in range(sim_n)]
            x_pos = np.arange(sim_n)

            for ax_b, vals, comp, col in zip(
                    axes, [xs, ys, zs],
                    ["⟨X⟩", "⟨Y⟩", "⟨Z⟩"],
                    [CYAN, PURPLE, YELLOW]):
                ax_b.set_facecolor(SIDEBAR)
                bar_colours = [col if abs(v) > 0.01 else BORDER for v in vals]
                bars_b = ax_b.bar(x_pos, vals, color=bar_colours,
                                  edgecolor=BORDER, linewidth=0.7, alpha=alpha_b)
                ax_b.axhline(0, color=BORDER, lw=0.8)
                ax_b.set_ylim(-1.15, 1.15)
                ax_b.set_xticks(x_pos)
                ax_b.set_xticklabels(qubit_labels, fontfamily="monospace",
                                     fontsize=9, color=WHITE)
                ax_b.set_title(comp, color=col, fontsize=11,
                               fontfamily="monospace", pad=5)
                ax_b.tick_params(colors=DIM, labelsize=8)
                ...
                if not is_running:
                    ax_b.text(0.5, 0.5, "// run to confirm",
                              transform=ax_b.transAxes, color=BORDER,
                              fontsize=9, fontfamily="monospace", ha="center", va="center")

            fig.suptitle("// single-qubit Bloch vector components (reduced density matrix)",
                         color=GREEN, fontsize=8, fontfamily="monospace", x=0.02, ha="left", y=1.01)
            ...
            st.markdown(
                ...
                f"margin-top:0.6rem'>// |Bloch vector| = 1 → pure qubit state · "
                f"|Bloch vector| < 1 → mixed (entangled with others)</p>", unsafe_allow_html=True)
            cols_b = st.columns(sim_n)
            for qi, (col_b, bv) in enumerate(zip(cols_b, bloch_data)):
                length = float(np.sqrt(sum(v**2 for v in bv)))
                col_b.markdown(
                    f"<div style='background:{PANEL};border:1px solid {BORDER};"
                    f"border-left:3px solid {CYAN};border-radius:4px;"
                    f"padding:0.45rem 0.7rem;font-family:JetBrains Mono,monospace'>"
                    f"<div style='color:{DIM};font-size:0.58rem;letter-spacing:1px;"
                    f"text-transform:uppercase'>q{qi} · |r|</div>"
                    f"<div style='color:{YELLOW};font-size:0.95rem;font-weight:700'>{length:.4f}</div>"
                    f"<div style='color:{DIM};font-size:0.62rem'>"
                    f"X={bv[0]:+.3f} Y={bv[1]:+.3f} Z={bv[2]:+.3f}</div>"
                    f"</div>", unsafe_allow_html=True)
